...
const ReptileWeightLogsTable: FC<ReptileWeightLogsTableProps> = ({
  ...
  const handleSelectAllReptileWeightLogs = (event: ChangeEvent<HTMLInputElement>): void => {
  ...
  const handleSelectOneReptileWeightLog = (
    ...
    if (!selectedReptileWeightLogs.includes(reptileWeightLogId)) {
      setSelectedReptileWeightLogs((prevSelected) => [...prevSelected, reptileWeightLogId]);
    } else {
  ...
  const paginatedReptileWeightLogs = applyPagination(reptileWeightLogs, page, limit);
  ...
  const selectedAllReptileWeightLogs = selectedReptileWeightLogs.length === reptileWeightLogs.length;
  ...
  const getReptileFromReptileWeightLog: (reptileWeightLog: ReptileWeightLog) => Reptile = (
    reptileWeightLog,
  ) => reptiles.find((reptile) => reptile.id === reptileWeightLog.reptileID)!;
  ...
  return (
    <Card>
      ...
      {!selectedBulkActions && <CardHeader title='体重日志管理' />}
      <Divider />
      <TableContainer>
        <Table>
          <TableHead>
          ...
          <TableBody>
            {paginatedReptileWeightLogs.map((reptileWeightLog) => {
              ...
              return (
                <TableRow hover key={reptileWeightLog.id} selected={isReptileWeightLogSelected}>
                  <TableCell padding='checkbox'>
                    <Checkbox
                      color='primary'
                  ...
                  <TableCell>
                    <Typography
                      ...
                      noWrap
                    >
                      {getReptileFromReptileWeightLog(reptileWeightLog).gender === 'MALE'
                        ? '公'
                        : '母'}
                    </Typography>
                  </TableCell>
                  <TableCell>
                    <Typography
                      ...
                      noWrap
                    >
                      {(getReptileFromReptileWeightLog(reptileWeightLog).genies ?? []).map(
                  ...
                  <TableCell>
                    ...
                    >
                      {format(
                        new Date(reptileWeightLog.meteringDateTime!),
